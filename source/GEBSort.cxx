...
//                strcpy (Pars.ROOTFileOption, argv[j++]);
                ...
                j++;
                sscanf(argv[j++], "%hd", &Pars.ignoreThresholds);

                if (Pars.ignoreThresholds > 0) {
                    string outMessage;

                    switch (Pars.noCalib) {
                    case 1: outMessage = "Thresholds won't be applied to the raw and/or sorted tree(s)"; break;
                    case 2: outMessage = "No thresholds will be applied to any tree(s)"; break;
                    }
