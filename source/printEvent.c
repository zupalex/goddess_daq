  ...
  if (Clstr[iCluster].valid)
    {
      ...
      if (Clstr[iCluster].tracked == 1)
        {
          fprintf (fp, "/bestperm=%3i/", Clstr[iCluster].bestPermutation);
...
#if SIMULATED
          fprintf (fp, "/%2i ", Clstr[iCluster].intpts[i].origPos);
#endif
          fprintf (fp, ", ts=%lli; ", Clstr[iCluster].intpts[i].timestamp);

#if SIMULATED
          if (Clstr[iCluster].intpts[i].origPos == 0)
            {
              if (Clstr[iCluster].intpts[i].order == 0)
                {
                  fprintf (fp, "HIT");
...
printAllClusters (char *fn, char *str, CLUSTER_INTPTS Clstr[MAXCLUSTERHITS], int *nClusters)
{
      ...
      if (Clstr[i].valid)
        {
          ...
          for (j = 0; j < Clstr[i].ndet; j++)
            if (Clstr[i].intpts[j].origPos == 0)
              i3++;
#endif

          /* find first interation point */
...
#if SIMULATED
          /* count # original first interation points */
          /* we want one and only one! */

          iorig = 0;
          for (k = 0; k < Clstr[i].ndet; k++)
            if (Clstr[i].intpts[k].origPos == 0)
              iorig++;

          /* require one and only one equal to orig pos to declare success */

          if (Clstr[i].intpts[j].origPos == 0 && iorig == 1)
            i1++;
          else
            i2++;
#endif
  ...
  fprintf (fp, "\n");
  fprintf (fp, "We found %2i gamma rays", i0);
#if SIMULATED
  fprintf (fp, "should have found %2i, a difference of %2i", i3, i0 - i3);
  fprintf (fp, "\n");
  fprintf (fp, "Of those, we correctly identified %2i of them\n", i1);
  fprintf (fp, "    and we incorrectly identified %2i of them\n", i2);
  fprintf (fp, "\n");
  r1 = 100 * (float) i1 / (float) (i1 + i2);
  fprintf (fp, "___correct hit fraction: %5.1f %%\n", r1);
#endif
  fprintf (fp, "\n");
