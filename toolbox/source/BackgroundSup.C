...
/*get the trees*/
	...
	TH2D* GamMatrix = new TH2D("GamMatrix", "Gammas vs Ex : all detector", 350, -0.5, 7, 4950, 50, 5000);
	TH1D* GammaPlot = new TH1D("GammaPlot", "Coincidence 1", 4950, 50, 5000);
	TH1D* antiGammaPlot = new TH1D("antiGammaPlot", "Anti Gamma", 4950, 50, 5000);
	TH1D* BgSuppr = new TH1D("BgSuppr", "Background suppressed", 4950, 50, 5000);
	TH1D* GammaP = new TH1D("GammaP", "Coincidence 2", 4950, 50, 5000);
	TH1D* GammaOri = new TH1D("GammaOri", "Gamma Plot Original - All gammas", 4950, 50, 5000);
	TH1D* Copy = new TH1D("GammaOri", "Gamma Plot Original - All gammas", 4950, 50, 5000);
	TH1D* gamdet = new TH1D("gamdet", "Detector gamma", 120, 0, 119);

	/*Upstream Sx3*/
	TH1D* f1us = new TH1D("f1us", "Time All DetID", 1000, 0, 1000);
	TH1D* f2us = new TH1D("f2us", "detectors gammasphere", 110, 5, 115);
	...
	TH2D* GamvsExus = new TH2D("GamvsExus", "Sx3 U0 - Strip 2 & 3: Gam vs. Ex", 350, -0.5, 7, 4950, 50, 5000);
	TH1D* Gamdd = new TH1D("Gamdd", "134Xe(d,d) Gam ", 4950, 50, 5000);
	TH1D* Gamdp = new TH1D("Gamdp", "134Xe(d,p) Gam ", 4950, 50, 5000);
	TH1D* Multiplicityus = new TH1D("Multiplicityus", " Gamma Multiplicity distribution", 100, 0, 99);
	TH1D* Gamgate = new TH1D("Gamgate", " Gated on Gammas @ 288 ", 350, -0.5, 7);

	/*Downstream Sx3*/
	TH2D* TotvAngleds = new TH2D("TotvAngleds", "Tot vs. Angle", 600, 0, 200, 400, 0, 20);
	TH2D* GamvsExds = new TH2D("GamvsExds", "Gam vs. Ex", 4950, 50, 5000, 4000, 0, 14);
	...
	TH2D* GamvsExuq = new TH2D("GamvsExuq", "QQQ5 D: Gam vs. Ex", 350, -0.5, 7, 4950, 50, 5000);
	TH2D* GamvsExuqdigi = new TH2D("GamvsExuqdigi", "QQQ5 A: Gam vs. Ex", 350, -0.5, 7, 4950, 50, 5000);
	TH2D* GamMatrixq5 = new TH2D("GamMatrixq5", "Gammas vs Ex : QQQ5 A & C", 350, -0.5, 7, 4950, 50, 5000);
	TH2D* ExvAngleuq = new TH2D("ExvAngleuq", "Ex vs. Angle", 600, 0, 200, 160, -0.5, 7);
	TH1D* f2uq = new TH1D("f2uq", "QQQ5 - Gammas - Ex gated", 4950, 50, 5000);
	TH1D* f5uq = new TH1D("f5uq", "QQQ5 - Gammas - Ex gated", 4950, 50, 5000);
	TH1D* f6uq = new TH1D("f6uq", "QQQ5 - Gammas - Ex gated", 4950, 50, 5000);
	TH1D* f7uq = new TH1D("f7uq", "QQQ5 - Gammas - Ex gated", 4950, 50, 5000);
	TH1D* f8uq = new TH1D("f8uq", "QQQ5 - Gammas - Ex gated", 4950, 50, 5000);
	TH1D* f9uq = new TH1D("f9uq", "QQQ5 - Gammas - Ex gated", 4950, 50, 5000);
	TH1D* f10uq = new TH1D("f10uq", "QQQ5 - Gammas - Ex gated", 4950, 50, 5000);
	TH1D* f4uq = new TH1D("f4uq", "QQQ5 - Gammas - Ex gated", 4950, 50, 5000);
	TH1D* f3uq = new TH1D("f3uq", "Y projection Gam vs Ex", 4950, 50, 5000);
	TH1D* Gamdduq = new TH1D("Gamdd", "134Xe(d,d) Gam ", 4950, 50, 5000);
	TH1D* Gamdpuq = new TH1D("Gamdp", "134Xe(d,p) Gam ", 4950, 50, 5000);
	TH1D* Gamddud = new TH1D("Gamdd", "134Xe(d,d) Gam ", 4950, 50, 5000);
	TH1D* Gamdpud = new TH1D("Gamdp", "134Xe(d,p) Gam ", 4950, 50, 5000);
		...
		for (int j = 0; j < siVecsize; j++)
		{
			...
			*/
			...
			if (detid == 1200) {// &&(Strip!=38415 && Strip != 38429 && Strip != 38431)) {
				...
				test = 0.;
				//	counter = 0;
				//	cout << " Counter is zero : NEW EVENT " << endl;
				for (int h = 0; h < gamVecsize; h++) { // This is if there is a gamma
					...
					//gamE = (vectGamData->at(h).en) / 3.;

					

					//if ((((timediffs > 175 && timediffs < 250) || (timediffs > 65 && timediffs < 120))) && gamE > 200 && gamType == 150 && Ex > 1) Copy->Fill(gamE);

					// gamType == 1 turns ON the compton suppression       && (Ex > 3.099 && Ex < 3.301)    && (Ex > 2.899 && Ex < 3.151)
					if ((((timediffs > 182 && timediffs < 224) || (timediffs > 66 && timediffs < 94))) && gamE > 100 && gamType == 1 && Ex > 0.5) {
						//counter++;
						GamMatrixq5->Fill(Ex, (vectGamData->at(h).en) / 3.);
						GamMatrix->Fill(Ex, (vectGamData->at(h).en) / 3.);
						if(test != gamVecsize)gamdet->Fill(gamVecsize);
						GammaOri->Fill((vectGamData->at(h).en) / 3.);
						GamvsExuqdigi->Fill(Ex, (vectGamData->at(h).en) / 3.);

						//if ((((vectGamData->at(h).en) / 3.) > 285) && (((vectGamData->at(h).en) / 3.) < 300)) Gamgate->Fill(Ex);


						//cout << "Energy" << endl;
						//cout << (vectGamData->at(h).en) / 3. << endl;
						//if (test != gamVecsize)cout << "Gamvecsize   " << gamVecsize << "        OR directly    " << (vectGamData->size()) << endl;
						//counter += 1;
						//for (int i = 0; i < vectGamData->size(); i++) {
						//	printf(" % f\n", vectGamData->at(i).en);
					//	}
				//		cout << " Gamma Energy" << endl;
					//	cout << gamE << endl;
						//cout << "the counter" << endl;
						//cout << counter << endl;
						...
						for (int k = 0; k < gamVecsize; k++) {
							...
							//counter1 += 1;
							if ((gamE_h > 1059 && gamE_h < 1081) && gamNum_k) {
						...
						for (int k = 0; k < gamVecsize; k++) {
							...
							if ((gamE_h > 278 && gamE_h < 298) && gamNum_k) {
								...
								GammaP->Fill(gamE_k);
								f2us->Fill(gamNum);
								//Multiplicityus->Fill(gamVecsize);
							}
						}
					}// 150
					Multiplicityus->Fill(gamVecsize);
					//cout << " Multiplicity = " << gamVecsize << endl;
					if ((timediffs > 310 && timediffs < 380)) {
						//if (gamType == 1)antiGammaPlot->Fill(gamE);
						nope = 1;
					}
				}
			}
...
//					gamE = (vectGamData->at(h).en)/3.;
					...
					//if ((((timediffs > 400 && timediffs < 450))) && gamE > 100 && gamType == 1)Copy->Fill(gamE);
					
					// gamType == 1 turns ON the compton suppression       && (gamType == 1)
					if ((((timediffs > 400 && timediffs < 450))) && gamE > 100 && gamType == 1 && Ex > 0.5) {
						GammaOri->Fill((vectGamData->at(h).en) / 3.);

						GamMatrixq5->Fill(Ex, (vectGamData->at(h).en) / 3.);
						GamMatrix->Fill(Ex, (vectGamData->at(h).en) / 3.);
						if (test1 != gamVecsize)gamdet->Fill(gamVecsize);
						GamvsExuq->Fill(Ex, (vectGamData->at(h).en) / 3.);
						counter1 += 1;
						...
						for (int k = 0; k < gamVecsize; k++) {
							...
							if ((gamE_h > 2103 && gamE_h < 2140) && gamNum_k) {
								//if (j != k) continue; // don’t use the same gamma ray
								if (gamNum_h == gamNum_k) continue; // make sure it’s a different detector
								//truth += 1;
								GammaPlot->Fill(gamE_k);
					...
					if (timediffs < 400) {
						//	if (gamType == 1)antiGammaPlot->Fill(gamE_k);
						nope = 1;
					}
				}
			...
			if (Strip == 70401 || Strip == 70402 || Strip == 70688) {
				...
				for (int h = 0; h < gamVecsize; h++) { // This is if there is a gamma
					...
					timediffs = (double)vectGamData->at(h).time - (double)si->at(j).TimestampMaxLayer(1, false);
					timefilters->Fill(timediffs); // Will allow to filter gamma event
					if (((timediffs > 400 && timediffs < 500) && (gamType == 1)) && gamE > 200 && Ex > 1) Copy->Fill(gamE);

					// gamType == 1 turns ON the compton suppression && (Ex > 3.099 && Ex < 3.301)   && (Ex > 2.899 && Ex < 3.151)
					if (((timediffs > 398 && timediffs < 472) && (gamType == 1)) && gamE > 100 && Ex > 0.5) {
						GamvsExus->Fill(Ex, (vectGamData->at(h).en) / 3.);
						GamMatrix->Fill(Ex, (vectGamData->at(h).en) / 3.);
						GammaOri->Fill((vectGamData->at(h).en) / 3.);
						if (test2 != gamVecsize) gamdet->Fill(gamVecsize);

						if ((((vectGamData->at(h).en) / 3.) > 285) && (((vectGamData->at(h).en) / 3.) < 300)) Gamgate->Fill(Ex);

						if (gamVecsize == 1 || gamVecsize == 2 || gamVecsize == 3) {
							if (Ex < 6.364) Gamdp->Fill((vectGamData->at(h).en) / 3.);
					...
					if ((timediffs > 100 && timediffs < 175)) {
						//if (gamType == 1)antiGammaPlot->Fill(gamE_k); 
						nope = 1;
					}
				}
...
/*	cout << " Counter - QQQ5 event: " << counter << endl;
	...
	Multiplicityus->Draw();
	//GammaOri->SetLineColor(kBlue-3);
	//GammaOri->Draw();
	//antiGammaPlot->SetLineColor(kViolet-3);
	//antiGammaPlot->Draw("same");
	//BgSuppr->SetLineColor(kBlack);
	//BgSuppr->Draw("same");
	TCanvas* InfoDet = new TCanvas("InfoDet", "Detector GammaSphere", 400, 300);
	InfoDet->cd();
	//gamdet->SetLineColor(kBlack);
	Gamgate->Draw();


	int g288r = BgSuppr->Integral(278, 298);
	int g288b = BgSuppr->Integral(270, 273);
	int g288t = BgSuppr->Integral(294, 302);
	int g288 = g288r - (g288t);

	int g1514r = BgSuppr->Integral(1521, 1526);
	int g1514b = BgSuppr->Integral(1518, 1520);
	//int g1514t = BgSuppr->Integral(1527, 1530);
	int g1514 = g1514r - (g1514b);

	cout << " 288 keV   " << g288r << endl;

	TCanvas* Suppressed = new TCanvas("Suppressed", "Total Gamma with background suppression", 400, 300);
	Suppressed->cd();
	BgSuppr->Draw();

	TCanvas* coincidencesgamma = new TCanvas("coincidencesgamma", "Gamma Coincidences", 400, 300);
	coincidencesgamma->cd();
	//GammaPlot->SetLineColor(kGray+2);
	GammaP->SetLineColor(kMagenta - 9);
